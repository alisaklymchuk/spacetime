...
class FourCopyTimeEmb(torch.nn.Module):
    ...
    def forward(self, values, time_emb):
        values = values.repeat(1, 1, 30)
        return torch.cat((values, time_emb), -1)

class FourCopyMirrorTimeEmb(torch.nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, values, time_emb):
        values = values.repeat(1, 1, 60)
        return torch.cat((time_emb, values, time_emb), -1)
...
class ModifiedLinearEmbedding(Embedding):
    ...
    def initialize_layers(self):
        ...
        self.encoder = FourCopyMirrorTimeEmb()
